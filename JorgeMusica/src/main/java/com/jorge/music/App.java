...
import com.jorge.music.menu.MenuBuilder;
 ...
 * - Provide examples of pattern use (not necessarily good use) - Singleton, Strategy, Factory, DAO
 * - Provide an MVC example
 * - Demonstrate the use of parameters on the command line, JOpt library
 * - Demonstrate database access methods and patterns based on SQLite and java JDBC libraries
 * - Demonstrate Reflection, Exceptions, Lists, Maps, File handling
 * 
 * Eclipse usage notes
 * - if you see a message cannot resolve to type and it is one of your own classes after import
 * --A: try Menu, Project > Clean see if that resolves it
 * - if you have created folders or you expect to see log files in the log folder after running the code byt don't see
 * --A: try Meny, File > Refresh to refresh the Eclipse package explorer tree
 * 
 *****************************************************************/
...
public class App 
{
	 ...
	 * if you try you will get a NullPointerExecption
	 */
